#!/bin/sh
# Exercises the public Forward Auth identity projection across three HA pods.
set -eu
. "$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)/e2e-port-lock.sh"
root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
: "${E2E_PORT:=19880}"
: "${KIND_CLUSTER:=goauthy-forward-auth-ha-e2e}"
: "${K8S_NAMESPACE:=goauthy}"
: "${GOAUTHY_IMAGE:=goauthy:e2e}"
: "${KIND_NODE_IMAGE:=kindest/node:v1.36.1@sha256:3489c7674813ba5d8b1a9977baea8a6e553784dab7b84759d1014dbd78f7ebd5}"
: "${E2E_DIAGNOSTICS_DIR:=}"
for command in docker kind kubectl kustomize go grep sed nc seq sleep; do
...
cleanup() {
	...
	if [ "$status" -ne 0 ] && kubectl config get-contexts -o name | grep -qx "kind-$KIND_CLUSTER"; then
		if [ -n "$E2E_DIAGNOSTICS_DIR" ]; then
			mkdir -p "$E2E_DIAGNOSTICS_DIR"
			kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" get pods -o yaml >"$E2E_DIAGNOSTICS_DIR/pods.yaml" 2>&1 || true
			kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" get events --sort-by=.metadata.creationTimestamp >"$E2E_DIAGNOSTICS_DIR/events.txt" 2>&1 || true
			kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" get statefulset/goauthy -o yaml >"$E2E_DIAGNOSTICS_DIR/statefulset.yaml" 2>&1 || true
			for pod in goauthy-0 goauthy-1 goauthy-2; do
				kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" describe pod "$pod" >"$E2E_DIAGNOSTICS_DIR/$pod.describe.txt" 2>&1 || true
				kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" logs "pod/$pod" --all-containers=true >"$E2E_DIAGNOSTICS_DIR/$pod.current.log" 2>&1 || true
				kubectl --context "kind-$KIND_CLUSTER" -n "$K8S_NAMESPACE" logs "pod/$pod" --all-containers=true --previous >"$E2E_DIAGNOSTICS_DIR/$pod.previous.log" 2>&1 || true
			done
		fi
...
overlay=$(kustomize build "$root/deploy/e2e-forward-auth-ha")
if printf '%s\n' "$overlay" | grep -q 'GOAUTHY_PASSKEY_'; then
	echo 'overlay regression: Forward Auth HA must not configure passkeys' >&2
	exit 1
fi
if printf '%s\n' "$overlay" | grep -A1 'name: GOAUTHY_BOOTSTRAP_FORCE_MFA' | grep -qE 'value: ?"?true"?'; then
	echo 'overlay regression: forced MFA is invalid without passkey configuration' >&2
	exit 1
fi
printf '%s\n' "$overlay" | sed -e "s#goauthy:e2e#$GOAUTHY_IMAGE#g" -e "s#http://localhost:18080#http://localhost:$E2E_PORT#g" | kubectl --context "$context" apply -f -
...
run_forward_auth() {
	phase=$1
	GOAUTHY_E2E_FORWARD_AUTH_URLS="http://localhost:$E2E_PORT,http://localhost:$((E2E_PORT + 1)),http://localhost:$((E2E_PORT + 2))" \
	GOAUTHY_E2E_FORWARD_AUTH_PHASE="$phase" \
	GOAUTHY_E2E_FORWARD_AUTH_STATE_FILE="$temp_dir/forward-auth-state.json" \
	...
	GOAUTHY_E2E_BROWSER_REDIRECT_URI=http://localhost:5555/callback \
	GOAUTHY_E2E_FORWARD_AUTH_HEADERS=1 \
	go test -count=1 -v "$root/test/e2e/browser" -run '^TestForwardAuthHA$'
}
start_forwards
run_forward_auth initial
...
start_forwards
run_forward_auth post-restart
echo 'HA Forward Auth exact-three gate passed'
