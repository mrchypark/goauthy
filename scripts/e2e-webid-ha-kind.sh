#!/bin/sh
# Exercises enabled WebID output on all three HA pods, then after replacing one.
set -eu
. "$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)/e2e-port-lock.sh"

root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
: "${E2E_PORT:=18780}"
: "${KIND_CLUSTER:=goauthy-webid-ha-e2e}"
...
kustomize build "$root/deploy/e2e-webid-ha" | sed -e "s#goauthy:e2e#$GOAUTHY_IMAGE#g" -e "s#http://127.0.0.1:18080#http://127.0.0.1:$E2E_PORT#g" | kubectl --context "$context" apply -f -
...
run_webid() {
	phase=$1
	GOAUTHY_E2E_WEBID_URLS="http://127.0.0.1:$E2E_PORT,http://127.0.0.1:$((E2E_PORT + 1)),http://127.0.0.1:$((E2E_PORT + 2))" \
	GOAUTHY_E2E_WEBID_ISSUER="http://127.0.0.1:$E2E_PORT" \
	GOAUTHY_E2E_WEBID_SUBJECT=webid-subject \
	GOAUTHY_E2E_WEBID_PHASE="$phase" \
	go test -count=1 -v "$root/test/e2e/webid_ha" -run '^TestWebIDHA$'
}
...
start_forwards
run_webid post-restart
echo 'HA WebID exact-three gate passed'
