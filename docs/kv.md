...
목록 검색은 값/키 목록에만 `limit`과 `search`를 허용한다. 기본·최대 목록 크기는
1,000개, 검색어는 64바이트, JSON 요청 본문은 64KiB 이하이다. 검색은 리터럴 부분
문자열 비교다. 초과 크기·알 수 없는 필드·잘못된 JSON·중복/알 수 없는 쿼리를 거부한다.
